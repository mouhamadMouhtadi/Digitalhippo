...
const ThankYouPage = async ({ searchParams }: PageProps) => {
  ...
  //   find the id of the user made the order
  const orderUserId =
          ...
          <div className="lg:col-start-2">
            ...
            {order._isPaid ? (
              <p className="mt-2 text-base text-muted-foreground">
                Your order was processed and your asstss are available to
            ...
            <div className="mt-16 text-sm font-medium ">
              <div className="text-muted-foreground">Order nb.</div>
              <div className="mt-2 text-gray-900">{order.id}</div>
              <ul className="mt-6 divide-y divide-gray-200 border-t border-gray-200 text-sm font-medium text-muted-foreground">
                {/* map over the products that user has bought */}
                {(order.products as Product[]).map((product) => {
                  const label = PRODUCT_CATEGORIES.find(
                    (c: { value: string }) => c.value === product.category
                  )?.label;
